...
incremented by its position in the array: the first digit will be incremented by 1, 
...
const incrementer = (nums) => {
    ...
    for(let i = 0; i < nums.length; i++) {
        incNums.push(nums[i] + increment);
        increment += 1; 
    }

    const result = incNums.map(nums => {
        if(nums.toString().length > 1) {
            let numToStr = nums.toString();
            return parseInt(numToStr[numToStr.length - 1]);
        }
        return nums;
    });
